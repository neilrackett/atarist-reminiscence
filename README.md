...
On the title menu, **Options** holds the skill level, the screen mode, the
...
current run; `RS.CFG` sets where they start (see below).
...
| `screen`                   | ✓           | How 224 lines meet the 200-line screen: `fit` drops a row in eleven (default), `fill` crops 18 off the top and 6 off the bottom in play (12 and 12 elsewhere), `top` opens the top border, `full` both. Also under Options on the title menu |
| `refresh_rate`             | ✓           | Display refresh rate, `50` or `60` (default: the machine's own). 60Hz draws Fit taller on most monitors; overscan always runs at 50Hz. Also under Options on the title menu                                                                  |
...
| `music`                    | ✓           | YM chip music, if the tracks have been built (see above). Also under Options on the title menu                                                                                                                                               |
| `music_volume`             | ✓           | Chip music level, 0-100 (default 50, a little under the sampled effects). Also under Options on the title menu                                                                                                                               |
